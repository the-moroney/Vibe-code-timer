...
// Initialize the opening screen with the manifesto
function initOpeningScreen() {
  ...
  words.forEach(word => createBassWord(word, wrapper));
}
...
function prepareNext() {
  chimePlayed = false;
  const wrapper = document.getElementById('taskNameWrapper');
  wrapper.innerHTML = ''; // Clear previous words
  ...
  const words = tasks[currentIndex].name.split(' ');
  words.forEach(word => createBassWord(word, wrapper));
...
function createBassWord(text, container) {
  ...
  // Randomly stagger: left, center, or right
...
// Run the initialization function at startup
initOpeningScreen();